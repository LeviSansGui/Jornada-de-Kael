...
func game_over():
	if player_health < 1:
		...
		get_tree().change_scene_to_file("res://4.fim_de_jogo/cenas/cena_fim_de_jogo_3.tscn")
...
func take_damage(amount):
	player_health -= amount  # Reduz a vida
	hurted = true  # Marca como ferido para animação
	emit_signal("change_life", player_health)  # Atualiza o HUD
	knockback()  # Aplica o knockback
	$audio_dano.play()  # Toca o som de dano

	# Temporariamente desabilita a colisão para evitar dano repetido
	get_node("sofreu_dano/colisao").set_deferred("disable", true)
	await get_tree().create_timer(0.5).timeout  # 0.5 segundos de invulnerabilidade
	get_node("sofreu_dano/colisao").set_deferred("disable", false)
	hurted = false
